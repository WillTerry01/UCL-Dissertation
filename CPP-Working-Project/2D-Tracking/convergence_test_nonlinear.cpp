...
int main() {
    std::cout << "=== CNIS Convergence Test for Nonlinear Tracking ===" << std::endl;
    ...
    std::string method = config["bayesopt"]["consistency_method"].as<std::string>("nis4");
    ...
    std::cout << "Turn rate: " << turn_rate << std::endl;
    ...
    for (int step = 0; step < num_steps; ++step) {
        ...
        CNISResult result = computeCNISForRuns(
            all_states, all_measurements, dt_vec, dt, turn_rate,
    ...
    std::string csv_filename = "../2D-Tracking/Saved_Data/cnis_convergence_nonlinear.csv";
    ...
    try {
        std::string h5_filename = "../2D-Tracking/Saved_Data/cnis_convergence_nonlinear.h5";
