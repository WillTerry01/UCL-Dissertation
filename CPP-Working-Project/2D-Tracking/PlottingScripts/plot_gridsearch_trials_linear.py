...
DEFAULT_H5 = "../2D-Tracking/Saved_Data/2D_gridsearch_trials_nis4.h5"
DEFAULT_SCENARIO = "../scenario_linear.yaml"


def load_linear_grid_trials(h5_path=DEFAULT_H5):
    if not os.path.exists(h5_path):
        print(f"Error: Grid-search trials file not found: {h5_path}")
        print("Please run GridSearch_Tracking_linear first to generate results.")
...
def infer_grid(q, r, c):
    ...
    # If full grid present, reshape to (len(uq), len(ur))
...
def plot_3d_scatter(q, r, c, elev=30.0, azim=45.0, title_suffix=""):
    ...
    # Attempt to add a surface (sheet) over the points
    mappable = None
    try:
        uq, ur, grid = infer_grid(q, r, c)
        if grid is not None:
            # Build coordinate mesh that matches grid shape (uq, ur)
            ...
            # Fallback to triangulated surface for scattered data
    ...
    min_idx = np.argmin(c)
    ax.scatter([q[min_idx]], [r[min_idx]], [c[min_idx]], color='red', s=100, marker='*', label='Best', alpha=1.0)
    ax.set_xlabel('Q (Process Noise Intensity)')
    ax.set_ylabel('R (Measurement Noise Variance)')
    ax.set_zlabel('C (Consistency Metric)')
    ax.set_title(f'Linear Grid Search Trials (3D){title_suffix}')
    # Apply custom 3D view
    ...
    out = os.path.join(PLOTS_DIR, 'linear_gridsearch_trials_3d.png')
...
def plot_heatmap(q, r, c, yaml_path=DEFAULT_SCENARIO, title_suffix=""):
    ...
    if grid is None:
        ...
        Q, R = np.meshgrid(ur, uq)  # note: grid is (uq, ur)
        plt.figure(figsize=(8,6))
    ...
    if q_best is not None and r_best is not None and c_best is not None:
        ...
        best_text = ""
    plt.title(f'Linear Dense Grid{title_suffix}{best_text}')
    plt.grid(True, linestyle='--', alpha=0.2)
    plt.legend(loc='best')
    plt.tight_layout()
    os.makedirs(PLOTS_DIR, exist_ok=True)
    out = os.path.join(PLOTS_DIR, 'linear_gridsearch_heatmap.png')
    plt.savefig(out, dpi=300, bbox_inches='tight')
    print(f"Saved: {out}")
    plt.show()
...
def main():
    parser = argparse.ArgumentParser(description='Plot grid-search results for linear tracking scenario.')
