...
# Histogram for best test (by CNIS)
best_idx = int(np.argmin(cnis))
plt.figure(figsize=(8, 5))
plt.hist(chi2[best_idx, :], bins=30, alpha=0.8, color='indianred', edgecolor='k')
...
plt.xlabel("NIS (chi2)")
plt.ylabel("Count")
plt.title(f"Nonlinear NIS histogram for q={q[best_idx]:.3f}, R={r[best_idx]:.3f} (CNIS={cnis[best_idx]:.6f})")
plt.legend()
plt.tight_layout()
