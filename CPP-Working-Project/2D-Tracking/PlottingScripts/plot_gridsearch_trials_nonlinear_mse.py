...
DEFAULT_H5 = "../2D-Tracking/Saved_Data/2D_gridsearch_nonlinear_trials_mse.h5"
DEFAULT_SCENARIO = "../scenario_nonlinear.yaml"


def load_nonlinear_grid_trials_mse(h5_path=DEFAULT_H5):
    if not os.path.exists(h5_path):
        print(f"Error: Grid-search MSE file not found: {h5_path}")
        print("Please run GridSearch_Tracking_Nonlinear_MSE first to generate results.")
...
# Log-normalization helpers (safe for positive MSE)
...
def plot_3d_scatter(q, r, c, elev=30.0, azim=45.0, title_suffix=""):
    ...
    ax.set_title(f'Nonlinear Grid Search (Avg Position MSE){title_suffix}')
    ...
    out = os.path.join(PLOTS_DIR, 'nonlinear_gridsearch_trials_mse_3d.png')
...
def plot_heatmap(q, r, c, yaml_path=DEFAULT_SCENARIO, title_suffix=""):
    # Read YAML ranges (prefer mse_grid_search, then grid_search, else parameter bounds)
    ...
    if q_best is not None and r_best is not None and c_best is not None:
        best_text = f" - Best: Q={q_best:.6g}, R={r_best:.6g}, AvgMSE={c_best:.6g}"
    else:
        best_text = ""
    plt.title(f'Nonlinear Dense Grid (Avg Position MSE){title_suffix}{best_text}')
    plt.grid(True, linestyle='--', alpha=0.2)
    plt.legend(loc='best')
    plt.tight_layout()
    os.makedirs(PLOTS_DIR, exist_ok=True)
    out = os.path.join(PLOTS_DIR, 'nonlinear_gridsearch_mse_heatmap.png')
    plt.savefig(out, dpi=300, bbox_inches='tight')
    print(f"Saved: {out}")
    plt.show()


def plot_heatmap_thresholded(q, r, c, yaml_path=DEFAULT_SCENARIO, threshold=5.0, title_suffix=""):
    # Same axes range selection as plot_heatmap
    q_min = q_max = R_min = R_max = None
    try:
        with open(yaml_path, 'r') as yf:
            cfg = yaml.safe_load(yf)
    ...
    # Build LogNorm only up to threshold, and set 'over' color for > threshold values
    # Compute vmin from positive values below or equal to threshold
    vals = grid[np.isfinite(grid)]
    below = vals[vals > 0]
    if below.size == 0:
        vmin = 1e-12
    else:
        vmin = max(1e-12, float(np.nanmin(below)))
    norm = colors.LogNorm(vmin=vmin, vmax=threshold)

    cmap = plt.cm.viridis.copy()
    cmap.set_over('crimson')  # all values above threshold appear as this color

    plt.figure(figsize=(8, 6))
    im = plt.imshow(grid, origin='lower', aspect='auto',
                    extent=[R_min, R_max, q_min, q_max],
                    cmap=cmap, norm=norm, interpolation='nearest')
    plt.colorbar(im, label=f'Average Position MSE (log scale, > {threshold:g} shown as one color)', extend='max')

    plt.xlabel('R (Measurement Noise Variance)')
    plt.ylabel('Q (Process Noise Intensity)')
    plt.title(f'Thresholded Heatmap (Avg MSE; threshold={threshold:g}){title_suffix}')
    plt.grid(True, linestyle='--', alpha=0.2)
    plt.tight_layout()
    os.makedirs(PLOTS_DIR, exist_ok=True)
    out = os.path.join(PLOTS_DIR, 'nonlinear_gridsearch_mse_heatmap_thresholded.png')
    plt.savefig(out, dpi=300, bbox_inches='tight')
    print(f"Saved: {out}")
    plt.show()
...
def main():
    parser = argparse.ArgumentParser(description='Plot MSE grid-search results for nonlinear tracking scenario.')
    ...
    parser.add_argument('--h5', type=str, default=DEFAULT_H5, help='Path to MSE HDF5 file.')
    parser.add_argument('--scenario', type=str, default=DEFAULT_SCENARIO, help='Path to scenario YAML.')
    parser.add_argument('--threshold', type=float, default=1.0, help='Threshold for single-color cap in thresholded heatmap.')
    args = parser.parse_args()

    q, r, c = load_nonlinear_grid_trials_mse(args.h5)
    if q is None:
        return
