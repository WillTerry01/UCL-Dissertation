...
// Compute CNEES over all runs using full information matrix (nonlinear)
static double computeCNEES_nonlinear(
    ...
    double turn_rate)
{
    ...
    for (int run = 0; run < num_graphs; ++run) {
        ...
        if (!dt_vec.empty() && static_cast<int>(dt_vec.size()) == T - 1) {
            ...
            fg.runNonlinear(all_states[run], &all_measurements[run], dt);
        }
    ...
    if (!dt_vec.empty() && static_cast<int>(dt_vec.size()) == T - 1) {
        temp_fg.runNonlinear(all_states[0], &all_measurements[0], dt_vec, true);
    } else {
        temp_fg.runNonlinear(all_states[0], &all_measurements[0], dt);
    }
...
// Compute CNIS over all runs (nonlinear)
static double computeCNIS_nonlinear(
    const std::vector<std::vector<Eigen::Vector4d>> &all_states,
    const std::vector<std::vector<Eigen::Vector2d>> &all_measurements,
    const std::vector<double> &dt_vec,
    double dt,
    ...
    double optimizer_vel_std)
{
    ...
    double mean_nis = 0.0;
    for (int run = 0; run < num_graphs; ++run) mean_nis += nis_values[run];
    mean_nis /= num_graphs;
    ...
    int total_dof;
    if (consistency_method == "nis3") total_dof = dimZ; else if (consistency_method == "nis4") total_dof = dimZ - dimX; else total_dof = nz;
...
int main() {
    std::cout << "=== Bayesian Grid Search for Nonlinear Factor Graph Tracking ===" << std::endl;
    ...
    bool save_h5 = true;
    std::string out_h5 = "../2D-Tracking/Saved_Data/2D_gridsearch_nonlinear_trials.h5";
    ...
    // Optimizer settings (used for nis4)
    ...
    std::cout << "=== Nonlinear Grid Search ===" << std::endl;
    std::cout << "q in [" << q_min << ", " << q_max << "] with " << q_points << " points\n";
    std::cout << "R in [" << R_min << ", " << R_max << "] with " << R_points << " points\n";
    ...
    std::vector<double> obj_out; obj_out.reserve(q_out.capacity());
        ...
        for (int iR = 0; iR < R_points; ++iR) {
            ...
            if (consistency_method == "nis3" || consistency_method == "nis4") {
                metric = computeCNIS_nonlinear(all_states, all_measurements, dt_vec, dt, q, meas_std,
                                               consistency_method, turn_rate, opt_max_iters, opt_verbose, opt_init_mode, opt_pos_std, opt_vel_std);
            } else {
    ...
    if (save_h5) {
        try {
            ...
            hsize_t dims[1] = {static_cast<hsize_t>(q_out.size())};
            H5::DataSpace space(1, dims);
            auto qds = file.createDataSet("q_values", H5::PredType::NATIVE_DOUBLE, space);
            auto rds = file.createDataSet("r_values", H5::PredType::NATIVE_DOUBLE, space);
            auto ods = file.createDataSet("objective_values", H5::PredType::NATIVE_DOUBLE, space);
            qds.write(q_out.data(), H5::PredType::NATIVE_DOUBLE);
            rds.write(r_out.data(), H5::PredType::NATIVE_DOUBLE);
            ods.write(obj_out.data(), H5::PredType::NATIVE_DOUBLE);
            file.close();
    ...
    try {
        std::ofstream txt("../2D-Tracking/Saved_Data/2D_gridsearch_best_nonlinear.txt");
        txt << "Best q: " << best_q << "\n";
        txt << "Best R: " << best_R << "\n";
