classdef PlatformPredictionEdge < g2o.core.BaseBinaryEdge
    ...
    methods(Access = public)
        ...
        function initialEstimate(obj)
            ...
            %  Defining PHI and normalising to avoid discontinuities
            phi = obj.edgeVertices{1}.x(3);
            phi = g2o.stuff.normalize_theta(phi);

            % Precomputing the trig functions
            cos_phi = cos(phi);
            sin_phi = sin(phi);

            % Defining M
            M = obj.dT * [cos_phi -sin_phi 0;
                          sin_phi cos_phi  0;
                          0       0        1];

            % Initial Estimate of Xk+1
            obj.edgeVertices{2}.x = obj.edgeVertices{1}.x + (M * obj.z);
            obj.edgeVertices{2}.x(3) = g2o.stuff.normalize_theta(obj.edgeVertices{2}.x(3));
        end
        ...
        function computeError(obj)
            ...
            M = obj.dT * [cos_phi -sin_phi 0;
                          ...
                          0       0        1];
            % Defining dX
            dX = obj.edgeVertices{2}.x - obj.edgeVertices{1}.x;

            % Defining the error 
            obj.errorZ = (M \ dX) - obj.z;
        ...
        function linearizeOplus(obj)
            ...
            % Calculate position difference components
            dX = obj.edgeVertices{2}.x - obj.edgeVertices{1}.x;
            ...
            % First Jacobian
            obj.J{1} = [-cos_phi, -sin_phi, -dX(1)*sin_phi + dX(2)*cos_phi;
                          sin_phi, -cos_phi, -dX(1)*cos_phi - dX(2)*sin_phi;
                               0,        0,  -1] / obj.dT;

            % Second Jacobian computing the inverse of M explicitly
            obj.J{2} = 1/obj.dT * [ cos_phi, sin_phi, 0;
                                   -sin_phi, cos_phi, 0;
                                        0,       0,  1];
        end
    end    
